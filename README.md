...
2. 地点分类
  ...
  - 根据照片的定位信息，你可以浏览某一城市/区县的相关照片。

  - 如果你的照片遍布天南海北，你可以通过设定省份/城市/区县快速找到你想要的照片。
...
3. 人物分类
  ...
  - 应用会根据图片中是否包含人脸以及人脸的可能归属对图片进行分类。

  - 你可以修改默认名称以便以后更方便的查找。

  - 你还可以通过更复杂的组合条件搜索照片。例如，你可以搜索同时包含了你以及你女（男）朋友的照片。
...
1. Date classification
  ...
  - Images created/taken in the same day are sorted in the same folder. Double click one folder, you can see all the images of this day.

  - If you want to browser images taken during a certain peroid of date, choose start date and end date above, and push search button. 

2. Place classification

  - Using the location (GPS) information stored in photoes, you can browser photoes of specific city or district.

  - If you have travelled to so many cities that you cann't find your favourite one, don't worry. Search it by choosing province/city/district name listed in combo boxes.

3. People classification

  - Application can sort your photoes according to whether it contains human face or not and who this face belongs to.

  - You can rename default names to make it easier to search.

  - You can search for images by combining double searching conditions. For example, you can search for images that contains you and/or/not your girl/boy friend at the same time.

  - You can browser origin big image by double-click.

4. Thing classification

  - In this page, you can find your doggy or catty by searching corresponding tag.
...
安装完所需的依赖后，直接在本文件夹中运行`python Personal_Moments.py`即可。

你如果觉得软件过于丑陋且速度较慢，你还可以直接浏览IMAGE文件夹中已经进行分类整理的图片。
...
1. Thing classification function

2. More precise face classificatin function on Asian faces, and dynamic threshold adjustment function
