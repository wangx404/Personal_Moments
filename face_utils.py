...
FEATURE_DIM = 128
FEATURE_CSV_FILE = os.path.join("model", "feature_centers.csv")
FACE_NUMBER_TXT_FILE = os.path.join("model", "face_numbers.txt")
FACE_NAME_TXT_FILE = os.path.join("model", "face_names.txt")

dlib_face_detector = dlib.get_frontal_face_detector()
dlib_cnn_face_detector = dlib.cnn_face_detection_model_v1(os.path.join("model", "mmod_human_face_detector.dat"))
dlib_landmark_predictor = dlib.shape_predictor(os.path.join("model", "shape_predictor_5_face_landmarks.dat"))
dlib_face_recognizor = dlib.face_recognition_model_v1(os.path.join("model", "dlib_face_recognition_resnet_model_v1.dat"))
...
def load_face_numbers(txt_file=FACE_NUMBER_TXT_FILE):
    ...
    """
    if not os.path.exists(txt_file): # file doesn't exist
        return []
